...
export default function Map2({ restaurantLocations }) {
    ...
    useEffect(() => {
        ...
        const locations = restaurantLocations.map(rl => ({
            type: 'Feature',
            properties: { description: '<strong>' + rl.name + '</strong>' },
            geometry: {
                type: 'Point',
                'coordinates': [rl.mapCoordinates.split(',')[1], rl.mapCoordinates.split(',')[0]]
            }
        }));
        ...
        return (
            <Container style={{ backgroundColor: "#F5F5F4", borderRadius: "5px", width: "100%", minWidth: "35rem", marginTop: "20px" }} className="container-fluid">
